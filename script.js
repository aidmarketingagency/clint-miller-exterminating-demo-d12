...
(function(){
  // -- prefers-reduced-motion gate (covers all JS-driven animation) --
  var motionQuery = window.matchMedia ? window.matchMedia('(prefers-reduced-motion: reduce)') : { matches: false };
  function reducedMotion(){ return !!motionQuery.matches; }

  // -- SMS thread staged reveal + typing indicators, replayable (D4) --
  ...
  function playThread(){
    ...
    var seq = [
      ...
      { t: 3360, action: function(){ typers[2].classList.add('show'); } },
  ...
  // -- Reveal-on-scroll for sections (play once; must never leave content hidden) --
  ...
  // -- Animated count-up for the stat number --
  // Stat target: 800 (annual recurring pest control contract value per customer)
  // Re-arms on every scroll re-entry; manual replay button also works.
  ...
  function showStatFinal(){
    countRun++;
    if (statEl) statEl.innerHTML = '$<span class="cents">' + STAT_TARGET + '</span>';
  }
  ...
  function runCount(){
    ...
    function step(ts){
      ...
      if (statEl) statEl.innerHTML = '$<span class="cents">' + val + '</span>';
  ...
  // If no IntersectionObserver, show final immediately
  if (!('IntersectionObserver' in window)){
    showStatFinal();
    if (statReplayBtn) statReplayBtn.style.display = 'none';
  }

  // -- A1: Mobile sticky bar -- hide while CTA panel is in view --
  ...
  // Reduced motion on first paint: SMS thread fully shown immediately
